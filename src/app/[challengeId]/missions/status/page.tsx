...
// console.error('Error fetching data:', error);
 ...
 if (!completion) {
 return <Badge variant="outline" className="bg-surface-raised text-content-primary border-line text-xs px-2.5 py-0.5 sm:text-[10px] sm:px-1.5 sm:py-0">미완료</Badge>;
 }

 switch (completion.verification_status) {
 case 'approved':
 case 'auto_approved':
 return <Badge className="bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300 border-green-300 dark:border-green-800 hover:bg-green-100 dark:bg-green-900/20 text-xs px-2.5 py-0.5 sm:text-[10px] sm:px-1.5 sm:py-0">완료</Badge>;
 case 'pending':
 return <Badge variant="outline" className="bg-yellow-100 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-300 border-yellow-300 dark:border-yellow-800 text-xs px-2.5 py-0.5 sm:text-[10px] sm:px-1.5 sm:py-0"><span className="sm:hidden">검증 대기</span><span className="hidden sm:inline">대기중</span></Badge>;
 case 'rejected':
 return <Badge variant="destructive" className="bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-300 border-red-300 text-xs px-2.5 py-0.5 sm:text-[10px] sm:px-1.5 sm:py-0"><span className="sm:hidden">거부됨</span><span className="hidden sm:inline">거부</span></Badge>;
 default:
 return <Badge variant="outline" className="bg-surface-raised text-content-primary border-line text-xs px-2.5 py-0.5 sm:text-[10px] sm:px-1.5 sm:py-0">미완료</Badge>;
 }
 };
 ...
 className="px-3 py-1.5 md:px-4 md:py-2 border border-line rounded-md text-xs md:text-sm"
 >
 ...
 <Table>
 <TableHeader>
 <TableRow className="bg-surface-raised/50 hover:bg-surface-raised/50">
 ...
 return (
 <TableRow key={user.id} className="hover:bg-surface-raised/50">
 <TableCell className="py-3">
 <div>
 <div className="text-xs font-medium text-content-primary">{user.name}</div>
 <div className="text-[11px] text-content-tertiary">@{user.username}</div>
 </div>
 </TableCell>
 <TableCell className="py-3">
 <div className="flex items-center gap-2">
 <div className="flex-1 bg-surface-sunken rounded-full h-1.5 min-w-[50px]">
 <div
 className="bg-blue-600 h-1.5 rounded-full transition-all"
 style={{ width: `${completionRate}%` }}
 />
 </div>
 <span className="text-xs font-medium text-content-primary min-w-[35px]">{completionRate}%</span>
 </div>
 </TableCell>
 {missions.map((mission) => (
 ...
 <div className="mt-1.5 bg-surface-sunken rounded-full h-1.5">
 <div
 className="bg-blue-600 h-1.5 rounded-full transition-all"
 style={{ width: `${completionRate}%` }}
 />
 ...
 {missions.map((mission) => (
 <div key={mission.id} className="flex items-center justify-between py-1.5 border-b last:border-b-0 border-line-subtle">
 <div className="flex-1 min-w-0 mr-2">
 <p className="text-xs font-medium text-content-primary truncate leading-tight">{mission.title}</p>
 ...
 {users.length === 0 && (
 <div className="text-center py-8 md:py-12 text-sm md:text-base text-content-tertiary">
 참가자가 없습니다.
 </div>
 )}
 </div>
