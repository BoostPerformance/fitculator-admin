...
import DesktopRunning from '@/components/runningpage/desktopRunning';
import MobileRunning from '@/components/runningpage/mobileRunning';
...
export default function UserRunningDetailPage() {
  ...
  const { currentWeekIndex, lastWeekIndex } = useMemo(() => {
    if (!userData || !userData.weeklyRunnings.length)
      return { currentWeekIndex: 0, lastWeekIndex: 0 };
    const today = new Date();
    let currentIdx = 0;
    let lastIdx = Math.max(0, userData.weeklyRunnings.length - 2);

    userData.weeklyRunnings.forEach((week, idx) => {
    ...
    if (userData.weeklyRunnings.length === 1) currentIdx = lastIdx = 0;
    return { currentWeekIndex: currentIdx, lastWeekIndex: lastIdx };
  }, [userData]);

  const handleBack = () => router.push(`/${params.challengeId}/running`);

  useEffect(() => {
    const weeklyRecordId =
      userData?.weeklyRunnings?.[currentWeekIndex]?.recordId;
  ...
  const currentWeekData = userData.weeklyRunnings?.[currentWeekIndex] || {
    recordId: '',
    label: '데이터 없음',
    runningTypes: {},
    dailyRunnings: [],
  ...
  const weeklyRecordId = userData?.weeklyRunnings?.[currentWeekIndex]?.recordId;
