...
// console.error('기록 저장 실패:', error);
 ...
 if (rank === 2) return <FaMedal className="text-content-disabled" />;
 if (rank === 3) return <FaMedal className="text-orange-600" />;
 return <span className="text-content-tertiary">{rank}</span>;
 };
 ...
 <div className="text-content-tertiary text-headline font-bold mb-2">
 ...
 <div className="bg-surface rounded-lg shadow p-4 sm:p-3">
 <div className="flex justify-between items-center mb-4 sm:mb-3">
 <h2 className="text-lg sm:text-base font-semibold">벤치마크 목록</h2>
 <button
 onClick={() => {
 setEditingBenchmark(null);
 setIsModalOpen(true);
 ...
 {benchmarks?.length === 0 && (
 <p className="text-content-tertiary text-center py-8 sm:py-6 text-sm">
 벤치마크가 없습니다.
 </p>
 )}
 ...
 <div
 key={benchmark.id}
 className={`p-3 sm:p-4 border rounded-lg cursor-pointer hover:bg-surface-raised transition-colors min-h-[56px] ${
 ...
 <p className="text-xs sm:text-sm text-content-secondary line-clamp-2">{benchmark.description}</p>
 <p className="text-[10px] sm:text-xs text-content-tertiary mt-1">
 ...
 className="p-2 min-w-[36px] min-h-[36px] flex items-center justify-center text-red-500 dark:text-red-400 hover:bg-red-50 dark:bg-red-900/20 dark:hover:bg-red-900/30 rounded-lg transition-colors"
 >
 ...
 {leaderboardLoading ? (
 <div className="text-center py-8 text-content-tertiary">Loading...</div>
 ...
 <div key={record.id} className="bg-surface-raised rounded-lg p-4 border border-line-subtle">
 ...
 <div>
 <div className="font-medium text-sm">{record.users?.name}</div>
 ...
 ? 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300 dark:bg-green-900/30 dark:text-green-400'
 : record.verification_level === 'photo_verified'
 ? 'bg-blue-100 text-blue-800 dark:text-blue-300 dark:bg-blue-900/30 dark:text-blue-400'
 ...
 <th className="text-left py-2 text-sm text-content-secondary">순위</th>
 <th className="text-left py-2 text-sm text-content-secondary">이름</th>
 <th className="text-left py-2 text-sm text-content-secondary">나이</th>
 <th className="text-left py-2 text-sm text-content-secondary">기록</th>
 <th className="text-left py-2 text-sm text-content-secondary">날짜</th>
 <th className="text-left py-2 text-sm text-content-secondary">인증</th>
 <th className="text-left py-2 text-sm text-content-secondary">수정</th>
 </tr>
 </thead>
 <tbody>
 {leaderboardLoading ? (
 <tr>
 <td colSpan={6} className="text-center py-8 text-content-tertiary">Loading...</td>
 </tr>
 ) : leaderboard?.length === 0 ? (
 <tr>
 ...
 leaderboard?.map((record, index) => (
 <tr key={record.id} className="border-b hover:bg-surface-raised">
 <td className="py-2">
 <div className="flex items-center gap-2">
 {getRankIcon(index + 1)}
 </div>
 ...
 <div className="bg-surface rounded-lg shadow p-8 sm:p-6 text-center text-content-tertiary text-sm">
