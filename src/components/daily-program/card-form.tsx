...
export function CardForm({ card, programId, programTitle, programStatus, challengeId, date, onSave, onCancel, hideCheckHint, onDismissCheckHint }: CardFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
      <div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="코칭 팁 (선택)"
          rows={2}
          className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white resize-none"
        />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <label className={`flex items-center gap-1.5 text-sm ${hasCheck ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-600'}`}>
          <input
            type="checkbox"
        ...
        <div className="flex items-center gap-1.5">
          <label className={`text-sm ${hasCheck ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-600'}`}>점수</label>
          <input
            type="number"
            value={scoreValue}
            ...
            min={0}
            disabled={!hasCheck}
            className="w-16 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white disabled:opacity-50 disabled:cursor-not-allowed"
          />
        </div>
      </div>
