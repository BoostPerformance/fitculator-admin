...
export function CalendarProgramChip({ program, onClick, onCardClick, compact = false }: CalendarProgramChipProps) {
  ...
  return (
    ...
    >
      ...
      {/* Empty state */}
      {cards.length === 0 && (
        <div className="rounded-lg border border-dashed border-gray-300 dark:border-gray-600 px-2 py-1.5">
          <span className="text-[10px] text-gray-400 dark:text-gray-500">카드 없음</span>
        </div>
      )}
    </div>
  );
