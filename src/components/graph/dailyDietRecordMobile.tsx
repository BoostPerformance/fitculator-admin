...
// console.error("Error in getWeekDates:", error);
 ...
 return (
 <div className="bg-surface sm:p-6 md:p-6 rounded-lg p-3 lg:py-7 col-span-3 pl-8 md:flex-col md:items-center lg:hidden md:hidden sm:block shadow-[0_0_12px_0_rgba(121,120,132,0.15)]">
 <h2 className="text-xl font-medium mb-6 text-content-secondary text-left">
 일별 식단 기록 현황
 </h2>
 ...
 {days.map((day, idx) => (
 <div key={idx} className="w-6 text-center text-content-tertiary">
 {day}
 </div>
 ))}
 </div>
 <div className="text-content-tertiary sm:hidden">총 업로드</div>
 </div>
 </div>
 ...
 <span className="text-content-secondary sm:text-label font-medium md:text-label font-medium">
 {activity.users.name}
 </span>
 </div>
 ...
 ${hasRecord ? "bg-[#FAAA16]" : "bg-surface-raised"}`}
 onClick={async () => {
 try {
...
// console.error(
 ...
 <div className="w-full h-3 bg-surface-raised rounded-full relative overflow-hidden">
 {(() => {
