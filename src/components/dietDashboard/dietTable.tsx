...
const DietTable: React.FC<DietTableProps> = ({
 ...
 return (
 <div
 className="bg-surface rounded-xl shadow-sm border border-line p-4 mb-3"
 >
 {/* 헤더: 순위, 이름, ID */}
 ...
 <span className="text-xs text-content-tertiary">피드백 진행률</span>
 ...
 {/* 코치메모 버튼 */}
 <button
 className="w-full py-3 px-4 bg-surface-raised rounded-lg text-sm text-content-secondary hover:bg-surface-sunken active:bg-surface-sunken transition-colors flex items-center justify-between min-h-[48px]"
 onClick={(e) => {
 e.stopPropagation();
 handleModalOpen(data.participant);
 }}
 >
 <span className="text-content-tertiary">코치메모</span>
 <span className="text-content-primary dark:text-white truncate max-w-[60%] text-right">
 ...
 /* 데스크톱 테이블 뷰 */
 <table className="table-auto w-full bg-surface shadow rounded-lg">
 <thead>
 <tr className="bg-surface text-left text-title-lg font-medium text-content-disabled">
 ...
 {participantsList.map((data, index) => (
 <tr key={index} className="text-content-tertiary hover:bg-surface-raised">
 ...
 {participantsList.length === 0 && (
 <tr>
 <td colSpan={4} className="p-4 text-center text-content-tertiary">
 표시할 참가자 데이터가 없습니다.
 </td>
 </tr>
 )}
