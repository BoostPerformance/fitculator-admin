...
import { useRunningDataQuery, useSingleUserRunningQuery } from '../hooks/useRunningDataQuery';
import {
 WeekInfo,
 RunningItem,
 WeeklyChartData,
...
const MobileRunning: React.FC = () => {
 const { challengeId, userId } = useParams();
 const router = useRouter();
 const [userData, setUserData] = useState<RunningItem | null>(null);
 const [weekInfo, setWeekInfo] = useState<WeekInfo[]>([]);
 const [apiError, setApiError] = useState<string | null>(null);

 // React Query hooks for data fetching
 const { weeklyChart, leaderboard, isLoading: challengeLoading } = useRunningDataQuery(challengeId as string);
 const { data: userStats, isLoading: userLoading } = useSingleUserRunningQuery(userId as string, challengeId as string);

 const loading = challengeLoading || userLoading;

 const processUserRunningData = useCallback(() => {
 ...
 processUserRunningData();
 }
 }, [weeklyChart, leaderboard, userStats, processUserRunningData]);
 ...
 <div className="bg-surface shadow-md rounded-2xl p-4 space-y-3">
 ...
 <div
 key={idx}
 className="flex justify-between items-center p-3 rounded-xl bg-surface-raised hover:bg-blue-50 dark:bg-blue-900/20 transition"
 onClick={() => {
 if (week && week.label) {
 router.push(
 `/${userData.challenge_id}/running/${userData.userId}/${weekNumber}?label=${week.label}`
 );
 }
 }}
 >
 ...
 W{weekNumber} <br />
 <span className="text-xs text-content-disabled">{week.label}</span>
 </div>
 ...
 <button
 className="mt-6 text-content-disabled font-bold hover:font-extrabold"
 onClick={() => router.push(`/${challengeId}/running`)}
 >
 ← 목록으로 돌아가기
 </button>
 </div>
